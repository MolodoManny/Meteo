...
class HourlyCell : UICollectionView {
    
    static var reuseIdentifiere : String = " HourlyCell"
    ...
    func configure(with item: WeatherInfo){
        ...
        if let data = dateFormatterGet.date(from: item.time){
        ...
        tempLabel.text = String(item.temp.kelvinToCelciusConverter()) + " C "
    }
}
